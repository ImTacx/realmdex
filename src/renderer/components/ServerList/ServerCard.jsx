...
const DiscordIcon = () => (
  <svg width="16" height="16" viewBox="0 -28.5 256 256" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid">
    <g>
...
export const ServerCard = ({ game }) => {
    ...
    const fetchPlayerCount = async () => {
        try {
            const response = await fetch(game.playerCountUrl, { method: 'GET', mode: 'no-cors' });
            if (response.ok) {
    ...
    return (
        <div className={`${themes[currentTheme].card} rounded-xl shadow overflow-hidden hover:shadow-lg transition-all duration-300 hover:scale-[1.02] group`}>
            <div className="relative w-full pt-[56.25%] bg-gray-200">
                <img src={game.banner} alt={game.title} crossOrigin="anonymous"
                    className="absolute top-0 left-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"/>
                <div className="absolute top-2 left-2">
                    {game.discordLink && (
                        <button className="inline-block p-1 transition-colors duration-200" onClick={handleDiscordClick}>
                            <DiscordIcon />
                        </button>
                    )}
                </div>
